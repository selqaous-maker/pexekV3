...
export default function Home() {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#f4f8fc] text-[#081528]">
      ...
      <header className="site-header">
        <div className="container flex h-[76px] items-center justify-between">
          ...
          <nav className="hidden items-center gap-7 text-[0.86rem] font-medium lg:flex" aria-label="Primary navigation">
            <button onClick={() => scrollToId("solution")} className="nav-link">Solutions</button>
            <button onClick={() => scrollToId("how")} className="nav-link">How it works</button>
            <button onClick={() => scrollToId("industries")} className="nav-link">Industries</button>
        ...
        {menuOpen && (
          <div className="mobile-nav lg:hidden">
            {[['Solutions', 'solution'], ['How it works', 'how'], ['Industries', 'industries'], ['About', 'about']].map(([label, id]) => (
              <button key={id} onClick={() => { setMenuOpen(false); scrollToId(id); }} className="border-b border-[#17334e] py-4 text-left text-lg font-medium">{label}</button>
            ))}
          ...
          <div className="container relative grid items-center gap-12 pb-20 pt-12 lg:grid-cols-[1.04fr_0.96fr] lg:pb-28 lg:pt-20">
            <div className="relative z-10 max-w-2xl">
              ...
              <div className="mt-9 flex flex-col gap-3 sm:flex-row sm:items-center">
                ...
                <Button onClick={() => scrollToId("how")} variant="outline" className="h-13 rounded-md border-[#52708e] bg-transparent px-6 text-[0.91rem] font-semibold text-white hover:bg-white/10 hover:text-white">See How It Works <ArrowDownIcon /></Button>
              </div>
      ...
      <footer className="bg-[#020718] py-10 text-[#c6d4ce]"><div className="container flex flex-col gap-8 md:flex-row md:items-end md:justify-between"><div><div className="flex items-center gap-2 text-white"><img src="/assets/pexek-original-logo_3cca9eb7.png" alt="" className="size-8" /><span className="font-display font-semibold tracking-[-0.05em]">PEXEK</span></div><p className="mt-3 max-w-sm text-sm leading-6 text-[#9caea7]">Managed customer-conversation workflows with approved knowledge and human control.</p></div><div className="flex flex-wrap gap-x-6 gap-y-3 text-sm text-[#aebdb7]"><button onClick={() => scrollToId("solution")}>Solutions</button><button onClick={() => scrollToId("industries")}>Industries</button><button onClick={() => scrollToId("assessment")}>Contact</button><a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer">WhatsApp</a><a href={PEXEK_LINKEDIN_URL} target="_blank" rel="noopener noreferrer">PEXEK on LinkedIn</a><a href="mailto:hello@pexek.com">hello@pexek.com</a><a href="/privacy">Privacy</a><a href="/terms">Terms</a><span>© {new Date().getFullYear()} PEXEK</span></div></div></footer>
    </div>
  );
}
